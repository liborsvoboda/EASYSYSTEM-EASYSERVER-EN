...
namespace GLOBALNET.Controllers
{
    ...
    public class GLOBALNETLoginHistoryListApi : ControllerBase
    {
        ...
        public async Task<string> GetLoginHistory()
        {
            ...
            { data = new GLOBALNETContext().LoginHistoryLists.ToList(); }
        ...
        public async Task<string> GetLoginHistoryListByFilter(string filter)
        {
            ...
            {
                data = new GLOBALNETContext().LoginHistoryLists.FromSqlRaw("SELECT * FROM LoginHistoryList WHERE 1=1 AND " + filter.Replace("+"," ")).AsNoTracking().ToList();
            }
        ...
        public async Task<string> GetLoginHistoryListId([FromBody] IdFilter id)
        {
            ...
            { data = new GLOBALNETContext().LoginHistoryLists.Where(a => a.Id == id.Id).First(); }
            return JsonSerializer.Serialize(data);
        }
        ...
        /*
            ...
            public async Task<string> GetLoginHistoryName([FromBody] NameFilter Name)
            {
                ...
                using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions
                { IsolationLevel = IsolationLevel.ReadUncommitted }))
                { data = new GLOBALNETContext().LoginHistories.Where(a => a.UserName == Name.Name.ToString()).ToList(); }
                return JsonSerializer.Serialize(data);
            }
        */
    }
