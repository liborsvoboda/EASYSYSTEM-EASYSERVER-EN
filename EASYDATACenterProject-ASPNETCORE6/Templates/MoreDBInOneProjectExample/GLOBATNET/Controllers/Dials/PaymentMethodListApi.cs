...
namespace GLOBALNET.Controllers
{
    ...
    public class GLOBALNETPaymentMethodListApi : ControllerBase
    {
        ...
        public async Task<string> GetPaymentMethodList()
        {
            ...
            {
                data = new GLOBALNETContext().PaymentMethodLists.ToList();
            }
        ...
        public async Task<string> GetPaymentMethodListByFilter(string filter)
        {
            ...
            {
                data = new GLOBALNETContext().PaymentMethodLists.FromSqlRaw("SELECT * FROM PaymentMethodList WHERE 1=1 AND " + filter.Replace("+"," ")).AsNoTracking().ToList();
            }
        ...
        public async Task<string> GetPaymentMethodListKey(int id)
        {
            ...
            {
                data = new GLOBALNETContext().PaymentMethodLists.Where(a => a.Id == id).First();
            }
        ...
        public async Task<string> InsertPaymentMethodList([FromBody] PaymentMethodList record)
        {
            try
            {
                ...
                var data = new GLOBALNETContext().PaymentMethodLists.Add(record);
        ...
        public async Task<string> UpdatePaymentMethodList([FromBody] PaymentMethodList record)
        {
            try
            {
                var data = new GLOBALNETContext().PaymentMethodLists.Update(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { insertedId = record.Id, status = DBResult.success.ToString(), recordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { status = DBResult.error.ToString(), recordCount = result, ErrorMessage = string.Empty });
            }
        ...
        public async Task<string> DeletePaymentMethodList(string id)
        {
            try
            {
                ...
                var data = new GLOBALNETContext().PaymentMethodLists.Remove(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { insertedId = record.Id, status = DBResult.success.ToString(), recordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { status = DBResult.error.ToString(), recordCount = result, ErrorMessage = string.Empty });
