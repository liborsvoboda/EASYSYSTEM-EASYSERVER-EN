...
namespace GLOBALNET.Controllers
{
    ...
    public class GLOBALNETBranchListApi : ControllerBase
    {
        ...
        public async Task<string> GetBranchList()
        {
            ...
            {
                data = new GLOBALNETContext().BranchLists.ToList();
            }
        ...
        public async Task<string> GetBranchListByFilter(string filter)
        {
            ...
            {
                data = new GLOBALNETContext().BranchLists.FromSqlRaw("SELECT * FROM BranchList WHERE 1=1 AND " + filter.Replace("+"," ")).AsNoTracking().ToList();
            }
        ...
        public async Task<string> GetActiveBranch()
        {
            ...
            { data = new GLOBALNETContext().BranchLists.Where(a => a.Active == true).First(); }
            return JsonSerializer.Serialize(data);
        }
        ...
        public async Task<string> GetBranchListKey(int id)
        {
            BranchList data;
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted }))
            { data = new GLOBALNETContext().BranchLists.Where(a => a.Id == id).First(); }
            return JsonSerializer.Serialize(data);
        }
        ...
        public async Task<string> InsertBranchList([FromBody] BranchList record)
        {
            try
            {
                ...
                var data = new GLOBALNETContext().BranchLists.Add(record);
        ...
        public async Task<string> UpdateBranchList([FromBody] BranchList record)
        {
            try
            {
                var data = new GLOBALNETContext().BranchLists.Update(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { insertedId = record.Id, status = DBResult.success.ToString(), recordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { status = DBResult.error.ToString(), recordCount = result, ErrorMessage = string.Empty });
            }
        ...
        public async Task<string> DeleteBranchList(string id)
        {
            try
            {
                ...
                var data = new GLOBALNETContext().BranchLists.Remove(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { insertedId = record.Id, status = DBResult.success.ToString(), recordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { status = DBResult.error.ToString(), recordCount = result, ErrorMessage = string.Empty });
