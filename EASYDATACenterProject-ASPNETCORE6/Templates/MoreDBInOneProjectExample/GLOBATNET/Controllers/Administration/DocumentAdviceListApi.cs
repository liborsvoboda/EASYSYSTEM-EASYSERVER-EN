...
namespace GLOBALNET.Controllers
{
    ...
    public class GLOBALNETDocumentAdviceListApi : ControllerBase
    {
        ...
        public async Task<string> GetDocumentAdviceList()
        {
            ...
            {
                data = new GLOBALNETContext().DocumentAdviceLists.ToList();
            }
        ...
        public async Task<string> GetDocumentAdviceListByFilter(string filter)
        {
            ...
            {
                data = new GLOBALNETContext().DocumentAdviceLists.FromSqlRaw("SELECT * FROM DocumentAdviceList WHERE 1=1 AND " + filter.Replace("+"," ")).AsNoTracking().ToList();
            }
        ...
        public async Task<string> GetDocumentAdviceListType(string documentType, int branchId)
        {
            ...
            {
                data = new GLOBALNETContext().DocumentAdviceLists.Where(a => a.DocumentType == documentType && a.Active == true && a.BranchId == branchId &&
        ...
        public async Task<string> InsertDocumentAdviceList([FromBody] DocumentAdviceList record)
        {
            try
            {
                ...
                var data = new GLOBALNETContext().DocumentAdviceLists.Add(record);
        ...
        public async Task<string> UpdateDocumentAdviceList([FromBody] DocumentAdviceList record)
        {
            try
            {
                var data = new GLOBALNETContext().DocumentAdviceLists.Update(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { insertedId = record.Id, status = DBResult.success.ToString(), recordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { status = DBResult.error.ToString(), recordCount = result, ErrorMessage = string.Empty });
            }
        ...
        public async Task<string> DeleteDocumentAdviceList(string id)
        {
            try
            {
                ...
                var data = new GLOBALNETContext().DocumentAdviceLists.Remove(record);
                int result = await data.Context.SaveChangesAsync();
                if (result > 0) return JsonSerializer.Serialize(new DBResultMessage() { insertedId = record.Id, status = DBResult.success.ToString(), recordCount = result, ErrorMessage = string.Empty });
                else return JsonSerializer.Serialize(new DBResultMessage() { status = DBResult.error.ToString(), recordCount = result, ErrorMessage = string.Empty });
