...
namespace LICENSESRV.Controllers
{
    [Authorize]
    [ApiController]
    [Route("LICENSESRVCalendar")]
    public class LICENSESRVCalendarApi : ControllerBase
    {
        [HttpGet("/LICENSESRVCalendar/{userId}")]
        public async Task<string> GetCalendarById(int userId)
        {
            ...
            {
                data = new LICENSESRVContext().Calendars.Where(a => a.UserId == userId).ToList();
            }

            return JsonSerializer.Serialize(data);
        }

        [HttpPost("/LICENSESRVCalendar")]
        ...
        public async Task<string> InsertOrUpdateCalendar([FromBody] DBModel.Calendar record)
        {
            ...
            {
                int result = 0;
                using (var db = new LICENSESRVContext())
                {
