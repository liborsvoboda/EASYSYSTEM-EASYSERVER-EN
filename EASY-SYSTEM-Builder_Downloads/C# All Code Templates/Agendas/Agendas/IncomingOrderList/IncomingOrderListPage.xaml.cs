    ...
    public partial class IncomingOrderListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try
            {
                ...
                DocumentAdviceList = await ApiCommunication.GetApiRequest<DocumentAdviceList>(ApiUrls.DocumentAdviceList, "incomingOrder/" + defaultAddress.Id, App.UserData.Authentification.Token);
                ...
                IncomingOrderList = await ApiCommunication.GetApiRequest<List<IncomingOrderList>>(ApiUrls.IncomingOrderList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            {
                DBResultMessage dBResult = await ApiCommunication.DeleteApiRequest(ApiUrls.IncomingOrderList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try
            {
                ...
                {
                    dBResult = await ApiCommunication.PutApiRequest(ApiUrls.IncomingOrderList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await ApiCommunication.PostApiRequest(ApiUrls.IncomingOrderList, httpContent, null, App.UserData.Authentification.Token); }
                ...
                if (dBResult.recordCount > 0)
                {
                    ...
                    dBResult = await ApiCommunication.DeleteApiRequest(ApiUrls.IncomingOrderItemList, dBResult.status, App.UserData.Authentification.Token);
                    ...
                    dBResult = await ApiCommunication.PutApiRequest(ApiUrls.IncomingOrderItemList, httpContent, null, App.UserData.Authentification.Token);
        ...
        private async void SetRecord(bool showForm, bool copy = false) {
            try
            {
                ...
                if (showForm)
                {
                    ...
                    else { DocumentItemList = await ApiCommunication.GetApiRequest<List<DocumentItemList>>(ApiUrls.IncomingOrderItemList, originalDocumentNumber, App.UserData.Authentification.Token); }
        ...
        private async void SetSubListsNonActiveOnNewItem(bool newItem) {
            if (newItem)
            {
                AddressList = (await ApiCommunication.GetApiRequest<List<AddressList>>(ApiUrls.AddressList, null, App.UserData.Authentification.Token)).Where(a => a.Active && (a.AddressType == "all" || a.AddressType == "customer")).ToList();
            ...
            {
                AddressList = (await ApiCommunication.GetApiRequest<List<AddressList>>(ApiUrls.AddressList, null, App.UserData.Authentification.Token)).Where(a => a.AddressType == "all" || a.AddressType == "customer").ToList();
        ...
        private async void ImportOffer() {
            try
            {
                ...
                IncomingOrderList = await ApiCommunication.GetApiRequest<List<IncomingOrderList>>(ApiUrls.IncomingOrderList, null, App.UserData.Authentification.Token);
                IncomingOrderList.ForEach(record =>
                {
