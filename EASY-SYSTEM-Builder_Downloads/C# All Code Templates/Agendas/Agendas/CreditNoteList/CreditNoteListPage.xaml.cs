    ...
    public partial class CreditNoteListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            try
            {
                ...
                CreditNoteList = await ApiCommunication.GetApiRequest<List<CreditNoteList>>(ApiUrls.CreditNoteList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        public async void DeleteRecord() {
            ...
            {
                DBResultMessage dBResult = await ApiCommunication.DeleteApiRequest(ApiUrls.NotesList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try
            {
                ...
                {
                    dBResult = await ApiCommunication.PutApiRequest(ApiUrls.CreditNoteList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await ApiCommunication.PostApiRequest(ApiUrls.CreditNoteList, httpContent, null, App.UserData.Authentification.Token); }
                ...
                if (dBResult.recordCount > 0)
                {
                    ...
                    dBResult = await ApiCommunication.DeleteApiRequest(ApiUrls.CreditNoteItemList, dBResult.status, App.UserData.Authentification.Token);
                    ...
                    dBResult = await ApiCommunication.PutApiRequest(ApiUrls.CreditNoteItemList, httpContent, null, App.UserData.Authentification.Token);
        ...
        private async void SetRecord(bool showForm, bool copy = false) {
            ...
            if (showForm)
            {
                ...
                else { DocumentItemList = await ApiCommunication.GetApiRequest<List<DocumentItemList>>(ApiUrls.CreditNoteItemList, originalDocumentNumber, App.UserData.Authentification.Token); }
        ...
        private async void ImportInvoice() {
            try
            {
                ...
                CreditNoteList = await ApiCommunication.GetApiRequest<List<CreditNoteList>>(ApiUrls.CreditNoteList, null, App.UserData.Authentification.Token);
                CreditNoteList.ForEach(record =>
                {
