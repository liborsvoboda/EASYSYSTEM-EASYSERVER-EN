...
namespace GlobalNET.Pages {
    ...
    public partial class ReceiptListPage : UserControl {
        ...
        public async Task<bool> LoadDataList() {
            ...
            List<ReceiptList> ReceiptList = new List<ReceiptList>();
            ...
            try
            {
                ...
                DocumentAdviceList = await ApiCommunication.GetApiRequest<DocumentAdviceList>(ApiUrls.DocumentAdviceList, "receipt/" + defaultAddress.Id, App.UserData.Authentification.Token);
                ...
                ReceiptList = await ApiCommunication.GetApiRequest<List<ReceiptList>>(ApiUrls.ReceiptList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token);
        ...
        private void DgListView_Translate(object sender, EventArgs ex) {
            try
            {
                ((DataGrid)sender).Columns.ToList().ForEach(e =>
                {
                    ...
                    else if (headername == "IssueDate") { e.Header = Resources["issueDate"].ToString(); (e as DataGridTextColumn).Binding.StringFormat = "dd.MM.yyyy"; e.CellStyle = DatagridStyles.gridTextRightAligment; e.DisplayIndex = 1; }
        ...
        public async void DeleteRecord() {
            ...
            {
                DBResultMessage dBResult = await ApiCommunication.DeleteApiRequest(ApiUrls.ReceiptList, selectedRecord.Id.ToString(), App.UserData.Authentification.Token);
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e) {
            try
            {
                ...
                {
                    dBResult = await ApiCommunication.PutApiRequest(ApiUrls.ReceiptList, httpContent, null, App.UserData.Authentification.Token);
                }
                else { dBResult = await ApiCommunication.PostApiRequest(ApiUrls.ReceiptList, httpContent, null, App.UserData.Authentification.Token); }
                ...
                if (dBResult.recordCount > 0)
                {
                    ...
                    dBResult = await ApiCommunication.DeleteApiRequest(ApiUrls.ReceiptItemList, dBResult.status, App.UserData.Authentification.Token);
                    ...
                    dBResult = await ApiCommunication.PutApiRequest(ApiUrls.ReceiptItemList, httpContent, null, App.UserData.Authentification.Token);
        ...
        private async void SetRecord(bool showForm, bool copy = false) {
            ...
            if (showForm)
            {
                ...
                else { DocumentItemList = await ApiCommunication.GetApiRequest<List<DocumentItemList>>(ApiUrls.ReceiptItemList, originalDocumentNumber, App.UserData.Authentification.Token); }
        ...
        private async void SetSubListsNonActiveOnNewItem(bool newItem) {
            if (newItem)
            {
                ...
                AddressList = (await ApiCommunication.GetApiRequest<List<AddressList>>(ApiUrls.AddressList, null, App.UserData.Authentification.Token)).Where(a => a.Active && (a.AddressType == "all" || a.AddressType == "supplier")).ToList();
        ...
        private async void ImportInvoice() {
            try
            {
                ...
                ReceiptList = await ApiCommunication.GetApiRequest<List<ReceiptList>>(ApiUrls.ReceiptList, null, App.UserData.Authentification.Token);
                ReceiptList.ForEach(record =>
                {
        ...
        private async void ShowReceipt() {
            try
            {
                ...
                BranchList defaultAddress = new BranchList();
                defaultAddress = await ApiCommunication.GetApiRequest<BranchList>(ApiUrls.BranchList, "Active", App.UserData.Authentification.Token);
                DocumentAdviceList = await ApiCommunication.GetApiRequest<DocumentAdviceList>(ApiUrls.DocumentAdviceList, "creditNote/" + defaultAddress.Id, App.UserData.Authentification.Token);
                if (DocumentAdviceList == null) { await MainWindow.ShowMessage(true, Resources["documentAdviceNotSet"].ToString()); }
                cb_totalCurrency.ItemsSource = CurrencyList = await ApiCommunication.GetApiRequest<List<CurrencyList>>(ApiUrls.CurrencyList, null, App.UserData.Authentification.Token);
                cb_notes.ItemsSource = NotesList = await ApiCommunication.GetApiRequest<List<NotesList>>(ApiUrls.NotesList, null, App.UserData.Authentification.Token);
                cb_unit.ItemsSource = UnitList = await ApiCommunication.GetApiRequest<List<UnitList>>(ApiUrls.UnitList, null, App.UserData.Authentification.Token);
