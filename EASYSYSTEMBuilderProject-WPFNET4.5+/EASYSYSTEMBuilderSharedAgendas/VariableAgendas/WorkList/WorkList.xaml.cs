...
namespace PRUVODKY
{
    public partial class WorkListPage : UserControl
    {
        ...
        public ExtendedWorkList selectedRecord = new ExtendedWorkList();
        ...
        public async Task<bool> LoadDataList()
        {
            ...
            try
            {
                if (MainWindow.serviceRunning) tempPersonList = await ApiCommunication.GetApiRequest<List<PersonList>>(ApiUrls.PRUVODKYPersonList, null, App.UserData.Authentification.Token);
                PersonalList.Clear();
                ...
                OperationList = await ApiCommunication.GetApiRequest<List<OperationList>>(ApiUrls.PRUVODKYOperationList, null, App.UserData.Authentification.Token);
                WorkPlaceList.Clear();
                ...
                tempWorkList = await ApiCommunication.GetApiRequest<List<WorkList>>(ApiUrls.PRUVODKYWorkList, null, App.UserData.Authentification.Token);
        ...
        private void DgListView_Translate(object sender, EventArgs ex)
        {
            ((DataGrid)sender).Columns.ToList().ForEach(e =>
            {
                ...
                if (headername == "WorkPlace") { e.Header = Resources["workPlace"].ToString(); e.CellStyle = Styles.gridTextRightAligment; }
                else if (headername == "PersonalNumber") e.Header = Resources["personalNumber"].ToString();
                else if (headername == "OperationNumber") { e.Header = Resources["operationNumber"].ToString(); e.CellStyle = Styles.gridTextRightAligment; }
                else if (headername == "Date") e.Header = Resources["date"].ToString();
                else if (headername == "WorkTime") { e.Header = Resources["workTime"].ToString(); e.CellStyle = Styles.gridTextRightAligment; }
                else if (headername == "Pcs") { e.Header = Resources["pcs"].ToString(); e.CellStyle = Styles.gridTextRightAligment; }
                else if (headername == "Amount") { e.Header = Resources["amount"].ToString(); e.CellStyle = Styles.gridTextRightAligment; }
                else if (headername == "FullName") { e.DisplayIndex = 3; e.Header = Resources["fullName"].ToString(); }
                else if (headername == "WorkPower") { e.DisplayIndex = 9; e.Header = Resources["workPower"].ToString(); e.CellStyle = Styles.gridTextRightAligment; }
                else if (headername == "Timestamp") { e.Header = Resources["timestamp"].ToString(); e.CellStyle = Styles.gridTextRightAligment; }
        ...
        private async void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                { dBResult = await ApiCommunication.PutApiRequest(ApiUrls.PRUVODKYWorkList, httpContent, null, App.UserData.Authentification.Token); ;
                } else { dBResult = await ApiCommunication.PostApiRequest(ApiUrls.PRUVODKYWorkList, httpContent, null, App.UserData.Authentification.Token); ; }
