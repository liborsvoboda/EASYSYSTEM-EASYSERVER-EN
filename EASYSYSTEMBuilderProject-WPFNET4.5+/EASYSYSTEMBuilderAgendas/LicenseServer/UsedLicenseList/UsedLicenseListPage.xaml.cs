...
namespace GlobalNET.Pages
{
    public partial class UsedLicenseListPage : UserControl
    {
        ...
        public async Task<bool> LoadDataList()
        {
            ...
            try { if (MainWindow.serviceRunning) addressList = await ApiCommunication.GetApiRequest<List<AddressList>>(ApiUrls.GlobalNETAddressList, null, App.UserData.Authentification.Token); }
            catch (Exception autoEx) {SystemFunctions.SaveSystemFailMessage(SystemFunctions.GetExceptionMessages(autoEx));}
            try { if (MainWindow.serviceRunning) itemList = await ApiCommunication.GetApiRequest<List<ItemList>>(ApiUrls.GlobalNETItemList, null, App.UserData.Authentification.Token); }
            catch (Exception autoEx) {SystemFunctions.SaveSystemFailMessage(SystemFunctions.GetExceptionMessages(autoEx));}
            try { if (MainWindow.serviceRunning) UsedLicenseLists = await ApiCommunication.GetApiRequest<List<UsedLicenseList>>(ApiUrls.GlobalNETUsedLicenseList, (dataViewSupport.AdvancedFilter == null) ? null : "Filter/" + WebUtility.UrlEncode(dataViewSupport.AdvancedFilter.Replace("[!]", "").Replace("{!}", "")), App.UserData.Authentification.Token); }
            catch (Exception autoEx) {SystemFunctions.SaveSystemFailMessage(SystemFunctions.GetExceptionMessages(autoEx));}
            try
            {
